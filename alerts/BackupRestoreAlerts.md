...
| `kVixError` | The alert with the error code is raised in the following scenarios: The {{site.data.keyword.baas_full_notm}} node cannot resolve the DNS name of the ESX host. The {{site.data.keyword.baas_full_notm}} node cannot connect to the ESX host over port 902. When the {{site.data.keyword.baas_full_notm}} node connecting to the VMDK to read the changed blocks through the ESX host failed. | |
