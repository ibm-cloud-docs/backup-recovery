...
|---|---|---|---|
|1. Setting up a Backup and Recovery Service (BRS) instance| | | |
| | UI | Deploy your Backup and Recovery instance (BRS instance) | |
| | UI | Create a Data Source Connection (for a VPC) in BRS | Data source connection is a tunnel between the BRS instance and the source server. The Data Source Connector is the thing that forms that tunnel. |
...
| | UI | Configure the Connector VSI in the BRS UI | Set up password and token: First, add the domain: ‘cloud.ibm.com’ works as a default. Then, paste the claim token from the ‘Create source connection’ modal. |
...
| |UI - need to check plug-in|Create and configure a new Protection group in BRS UI (Protection page)|Physical server > File - Add object|
...
1. Deploy a Backup and Recovery instance.
   ...
   1. Create a BRS [instance](/docs/backup-recovery?topic=backup-recovery-getting-started-backup-recovery#baas-provision-instance).
      ...
      1. Information: When following best practices, it is recommended that you create your BRS instance in the same location as your source server.
   2. Open your BRS instance and **Launch Dashboard**.


1. Create a Data Source Connection in BRS

   For this step, you are setting up a Data Source Connection from inside your BRS instance.

   1. In the BRS Dashboard, navigate to **System > Data Source Connections** and click **New Connection**.
...
1. Deploy a BRS Data Source Connector (Connector VSI)
   ...
   2. On the VSI provisioning page:
      1. Set the same location as you set for the BRS instance.
...
1. Access and configure Connector VSI in BRS UI
...
1. Create a Virtual Private Endpoint gateway (VPE) instance for the BRS instance
   ...
   4. Select the previously created BRS instance from the list.
   5. Click **Create**.
...
1. Download and install the BRS agent
...
1. Install the BRS agent to Source VSI
   ...
   1. (Optional) Verify a copy by going to the root folder (cd) and listing files (ls). You should be able to see the BRS agent installer.
...
## Register the source VSI to BRS
{: #baas-register-source-vsi}
{: step}

1. Select type and register Source VSI

   1. Return to the BRS instance Dashboard, and navigate to the Data Protection > Sources page.
...
## Set up Data Protection in BRS
{: #baas-set-up-data-protection}
{: step}

1. Set up a new Protection group

   1. In the BRS instance Dashboard, navigate to the **Data Protection > Protection** page
