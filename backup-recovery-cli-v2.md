...
### `ibmcloud backup-recovery access-token-generate --help`
{: #backup-recovery-cli-access-generate-help-command}

access-token-generate - No Privileges Required

The POST request returns an access token and type in the response that is generated by the BRS Cluster. The generated token is valid for 24 hours. If a request is made with an expired token, the `Token expired` error message is returned.
...
#### Command options
{: #backup-recovery-access-generate-cli-options}
...
`--otp-type` (string)
:   Specifies OTP type for MFA verification. `Totp` implies the code is TOTP. `Email` implies the code is email OTP. Allowable values are: Totp, Email.

`--password` (string)
:   Specifies the password of the BRS user account.

`--private-key` (string)
:   Specifies the matching private key of the above certificate.

`--username` (string)
:   Specifies the login name of the BRS user.

#### Example
{: #backup-recovery-access-generate-examples}

```sh
ibmcloud backup-recovery access-token-generate \
