...
Alerts are generated by managed clusters and aggregated in the Backup and Recovery Manager dashboard. Alerts displayed are specific to the selected region.
...
Once you have reviewed the alert, you can resolve the alert using the page's Resolution section. You can create a new alert resolution or attach an existing one in the Resolution section. Resolving an alert does not correct the underlying issue; it only marks the alert as addressed.
...
If you see an empty report, verify the region selection, IAM permissions, and cluster connectivity.
{: note}
