...
{{site.data.keyword.baas_full}} protects Kubernetes namespaces and PVCs by assigning them to a policy‑driven Protection Group with options like CSI snapshots, Auto Protect, label-based selection, policy reuse, and application quiescing to ensure consistent backups for stateful workloads.
...
Here is how to protect a namespace or cluster and schedule a backup:
...
To configure quiescing for a namespace:
