...
After you have protected your Kubernetes namespaces, you can recover them from IBM Backup and Recovery Service to either:
...
If you have selected a Protection Group, then all the namespaces backed up in the latest protection run of this Protection Group will be selected for recovery. The option to select these namespaces individually will be greyed out in the search result list. You can select Protection Groups and namespaces with snapshots stored in the same Storage Domain and belonging to the same source. Once you select a namespace or Protection Group, the **Storage Domain** and **Source** filters will automatically get locked.
{: note}

5. To recover from a different snapshot, perform the following steps depending on the object selected:
...
To choose a different snapshot for a Protection Group:
 ...
 - Optionally, you can also select the snapshot archived to the cloud by clicking the cloud icon if available in the `Location` option.
  ...
  - **Namespace Resources**: Allow you to select the namespace resources you want to recover. Enable the following options to customize these settings: Click the edit icon on **Namespace Resources**.
    ...
    - **Storage Class**: Enable the following options to customize these settings:
        ...
        - To map recovered PVCs to a different storage class:
           ...
           - To create additional mappings, click `+ Add`.
    ...
    - **Include or Exclude Labels**: Enable the **Enable PVC Inclusion/Exclusion** option and provide the following information:
       ...
       - Select `Include`, click `+ Add`, and provide the key and value of the label assigned to the PVCs you want to include.<br> Or <br> Select Exclude, click + Add, and provide the key and value of the label tagged to the PVCs you want to exclude.
...
If you have selected a Protection Group, then all the namespaces backed up in the latest protection run of this Protection Group will be selected for recovery. The option to select these namespaces individually will be greyed out in the search result list.
You can select Protection Groups and namespaces with snapshots stored in the same Storage Domain and belonging to the same source. Once you select a namespace or Protection Group, the `Storage Domain` and `Source` filters will automatically get locked.
{: note}
...
9. Optional. Edit the default Recovery Options:
   - **Rename**: Add Prefix and/or Suffix strings to the names of the new namespaces created by this task. For example, you could specify Prefix `Test_` and Suffix `_QA` to the namespace’s name `App` and when the namespace is recovered, the new namespace that is created is called `Test_App_QA`.
   - **Task Name**: Change the default name of the recovery task.
   - **Namespace Resources**: Allows you to select the namespace resources you want to recover. Enable the following options to customize these settings: Click the edit icon on **Namespace Resources**.
      ...
      - **Storage Class**: Enable the following options to customize these settings:
         ...
         - To map recovered PVCs to a different storage class:
            ...
            - This mapping ensures that any PVCs using the selected source storage class are recreated using the selected target storage class during recovery.
            - To create additional mappings, click `+ Add`.
   - **Additional capability — alternative region/zone recovery**: BRS also supports recovering a Kubernetes cluster namespace to an alternative region/zone. To utilize this feature:
      - Specify up to 1 region mapping and one or more zone mappings.
      - BRS will inspect the following resource types and modify region/zone information:
         - Storage Classes:
         ...
         - Secrets:
            ...
            - Only secrets with type:
               ...
               - pxd.portworx.com
         - Persistent Volumes: If a PV is being restored, BRS will modify:
            - If a PV is being restored, BRS will modify:
...
| Task Name | Specifies the name assigned to the migration task within the workflow. This helps identify and track the task during execution and troubleshooting. |
...
| Namespace Resources | Displays only the resources that actually exist in the source namespace and allows you to select which ones to migrate from a drop down list. This helps us restore only relevant objects from the source namespace. We can also select either PVCs and other related resources present in the namespace for Inclusion/Exclusion. |
