...
1. Verify VPE Gateway Settings Check your VPC configuration to help ensure that no VPE (Virtual Private Endpoint) gateway is enabled for the BRS (Backup and Recovery Service) instance.
2. Confirm that the Correct Connection Type Double-check that the connection type that is selected during setup matches your actual cluster:
   - IKS classic
   - IKS VPC
   - ROKS classic
   - ROKS VPC
...
5. Check the Data Source Connector Pod for Issues Inspect the connector pod in your Kubernetes/OpenShift cluster for problems during startup:
   ...
   - If you see IAM/permission errors that are tied to volume attachment, reset the infrastructure API key by running:
...
- Result: The scheduler recognizes the nodes in those zones, and successfully binds the existing VPC Block volumes to the DSC pods.
...
- Help ensure the Persistent Volume Claims (PVCs) and PVs associated with the old zones are deleted.
- Reinstall DSC. This allows the CSI driver to provision new volumes in the currently active zones.
...
If there is registration failure, verify that the IBM Cloud Backup and Recovery backup agent is running or not on the Kubernetes/OpenShift cluster.
...
Before diving into detailed logs, perform a quick high-level check to help ensure that the foundation of your integration is still valid:
