...
| `[/customcertstype=usercerts\|{{site.data.keyword.baas_full_notm}}customcerts]` | Specifies the type of custom certificates. Possible values include {{site.data.keyword.baas_full_notm}} Custom Certs and User Custom Certs. |
| `[/{{site.data.keyword.baas_full_notm}}customcertpath=<path-to-agent-certificate-in-{{site.data.keyword.baas_full_notm}}-format>` | To be used only when **/customcertstype = {{site.data.keyword.baas_full_notm}}customcerts**.<br><br>Specifies the directory path in which the {{site.data.keyword.baas_full_notm}} custom certificate for the agent is present. |
...
On Windows physical servers, the Backup agent logs are stored in C:/programData/{{site.data.keyword.baas_full_notm}}/.
