...
*   **Backing up AWS infrastructure** — For this functionality, the registered AWS Cloud source contains the Amazon EC2 and/or S3 to back up.
    ...
    The backup support for AWS S3 is an Early Access feature. Contact your {{site.data.keyword.baas_full_notm}} account team to enable the feature.
...
*   **Converting and cloning VMware VMs** to AWS — For this functionality, the registered AWS Cloud source is where the VMs are cloned to. Converting and cloning VMs occur during the following use cases: 
    ...
    *   Replicate and Clone VMs in Cloud Edition
    ...
    For registering an AWS source to a Cloud Edition {{site.data.keyword.baas_full_notm}}, you can use IAM: User or IAM: Role. If you use Cloud Edition, you can [Automate IAM Role Permission Assignment for {{site.data.keyword.baas_full_notm}} AWS Cloud Edition](/docs/backup-recovery?topic=backup-recovery-iam-docs-template&interface=ui).
...
2. In the {{site.data.keyword.baas_full_notm}} UI, select **Data Protection > Sources**.
...
6. Click the **CloudFormation Template** link to download the CFT file.
    ...
    See [Automate IAM Role Permission Assignment for {{site.data.keyword.baas_full_notm}} AWS Cloud Edition](/docs/backup-recovery?topic=backup-recovery-iam-docs-template&interface=ui) on AWS for details.
