...
2.  Install the agent on the physical server.
    ...
    Instructions for silent installation on Windows servers are available here: [Silently Install the Backup agent on Windows Physical Servers](https://docs.cohesity.com/6_6/Web/UserGuide/Content/Dashboard/Protection/AgentWindows.htm#Silently).
...
3.  Specify the hostname (FQDN) or server IP (IPv4 or IPv6) of the server to register. Cohesity recommends using the FQDN.
4.  Click **Register**.

    When registration finishes, a new entry appears in the physical section on the Sources page.


Now you can [create a Protection Group with the Registered Source](https://docs.cohesity.com/6_6/Web/UserGuide/Content/Dashboard/Protection/JobServerPhysical.htm).
...
*   During agent reregistration, you must use one of these options, **Apply settings for agent preserved on Cohesity UI** or **Preserve setting during uninstallation of an agent from the server**.
...
3.  Click **Apply the Settings for the agent preserved on the <date and time>** checkbox if you want to restore the server settings.
    ...
    During agent uninstallation of a Cohesity Windows or Linux agent, there is an option to preserve settings in the windows registry or config file for Linux. If you had selected the **Preserve Settings** option during the uninstall process then deselect the above option during the reregistration workflow.
